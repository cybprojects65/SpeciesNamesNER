...
public class Orchestrator {
	...
	public static void main(String[] args) throws Exception {
		...
		// Seconda parte dell'esecuzione del programma adibita all'annotazione delle tassonomie

		ASFAResearchObject researchObject = new ASFAResearchObject();
		
		 // Catturo il testo dal file JSON ottenuto dall'output di NLPHub
		researchObject.capture(caller.getOutputJsonFile().toString());
		researchObject.get();
		researchObject.enrich("TAXON");
		researchObject.materializeText(caller.getOutputAnnotationFile().toString()) ;
		researchObject.materializeJSON(caller.getOutputJsonFile().toString());
